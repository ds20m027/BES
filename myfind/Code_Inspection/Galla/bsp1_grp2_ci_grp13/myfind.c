...
/*
 * ### FB_TMG: Seit ISO-C:99 existier der Datentypen bool mit den
 * Werten true und false in stdbool.h
 */
#define TRUE 1
...
/*
 * ### FB_TMG: Parameter file_info sollten const struct stat * sein
 */
...
/*
 * ### FB_TMG: Und wer gibt den allozierten Speicher (ret_opt_on_mask(()) wieder
 * frei [-1]
 */
            return EXIT_FAILURE;
...
/*
 * ### FB_TMG: Fehlerbehandlung fflush(stdout) fehlt
 */
    return EXIT_SUCCESS;
}
...
/*
 * ### FB_TMG: Und warum leifern Sie hier EXIT_SUCCESS an das
 * environment? [-1]
 */
        return EXIT_SUCCESS;
    } else {
...
/*
 * ### FB_TMG: Wenn Sie VLAs verwenden, sparen Sie Sich den
 * malloc()/free() Tanz ...
 */
            ...
            errno = 0;
/*
 * ### FB_TMG: Kann mit beliebig tiefen Verzeichnishierarchien
 * umgehen, ohne, daß die filedescriptor table voll wird
 */
...
/*
 * ### FB_TMG: Und wer gibt den allozierten Speicher (malloc()) wieder
 * frei wenn do_file() (oder eine der darin aufgerufenen Funktionen
 * ein exit() macht? [-1]
 */
...
/*
 * ### FB_TMG: Die explizite (char *) cast sind  unnötig und sollte deshalb
...
/*
 * ### FB_TMG: Und warum machen wir das hier?
 */
...
/*
 * ### FB_TMG: Hmmmm ... - ENOENT bzw. ESRCH ... weisen aber auch darauf
 * hin, dass die Gruppe nicht gefunden wurde [-1]
 *
 * ERRORS
 *  0 or ENOENT or ESRCH or EBADF or EPERM or ...
...
/*
 * ### FB_TMG: Besser sizeof(_date) dann brauchen Sie die Konstante
...
/*
 * ### FB_TMG: _buffer haben Sie hier ja noch gar nicht realloc()ed ...
 */
            free(_buffer);
            free(_p_link_string);
            exit (EXIT_FAILURE);
...
/*
 * ### FB_TMG: Was genau machen Sie hier jetzt?
 */
 ...
 */
    ...
    free (_p_link_string);
/*
 * ### FB_TMG: Wurde vermutlich gar nicht realloc()ed ...
 */
    free (_buffer);
    return EXIT_SUCCESS;
}
...
static void print_usage(const char *message) {
/*
 * ### FB_TMG: Usage Meldungen haben den Programmnamen (i.e., argv[0]
 * und nicht hardcoded myfind) zu beinhalten - Rationale: Umbenennen
 * des Executables, Links, exec(), ...
 */
 ...
 */
/*
 * ### FB_TMG: Korrektes Pre-Processing der Argumente - Sehr schön
 * ...
 */
static int validate_options(const char * const *parms) {
    int _is_valid = FALSE;
...
/*
 * ### FB_TMG: Faür den Fall, dass es beim erstem Mal aufgrund von
 * Sonnenflecken nicht funktioniert hat?
 */
        _is_valid = ret_mask_to_opt_string(parms[_parms_index]);
...
/*
 * ### FB_TMG: Fehlerbhandlung strdup() fehlt [-1]
 * Warum duplizieren Sie den String hier überhaupt?
 */
    char *_name_file_name = strdup(file_name);
/*
 * ### FB_TMG: Hier sollten Sie FNM_NOESCAPE verwenden sonst verhält
 * sich Ihr myfind bei Filenamen mit '\' anders
 */
	/*
...
/*
 * ### FB_TMG: Und wer gibt den allozierten Speicher (strdup()) wieder
 * frei [-1]
 */
        exit (EXIT_FAILURE);
    }
...
/*
 * ### FB_TMG: Diese Schleife ist overengineered ...
 */
...
/*
 * ### FB_TMG: Wenn die Ausgabe auf stderr nicht funktioniert noch
 * etwas auf stderr auszugeben, macht eher wenig Sinn ...
 */
...
/*
 * ### FB_TMG: Das "normale" find(1) läßt Sie schon nach UID suchen,
 * die *nicht* in /etc/password sind [-1]
 */    
	...
	errno = _my_errno;                                                       
/*
 * ### FB_TMG: Was machen Sie hier? - Warum verleichen Sie nicht einfach
 * _p_pwd_entry->pw_uid bzw. _conv_uid mit file_info->st_uid? [-1]
 */    
...
/*
 * ### FB_TMG: Das "normale" find(1) läßt Sie schon nach UID suchen,
 * die *nicht* in /etc/password sind
 */    
...
/*
 * ### FB_TMG: Und was ist wenn das Argument zu -user "1234foobar"
 * ist? - Dann nehmen Sie 1234 als UID => 2. Argument von strtol()
 * sinnvoll benutzen [-1]
 */
