...
* \name do_file
*
* \brief checks the file and prints if it matches the supplied parameter set
*
* checks the file for the parameters sequentially.
* keeps track if there was an output command, if not then prints it at the end
*
* \param file_name		path of the file to check
* \param parmeters		parameter list
* \param set_offset     value of the offset to the first process parameter
*
* \return nothing
*
*/
...
* \name do_dir
*
* \brief crawls directory for files and subdirectories
*
* The function do_dir reads the entries of the directory dir_name
* and checks theese entries with do_file.
* When the directory cannot be openend, it prints an error message.
* checks for trailing / at the end of dirname to avoid // structures
* which it can resolve, but look ugly
*
* \param dir_name		name of the directory
* \param parameters		pointer to whole parameter set
* \param set_offset		value of the offset to the first process parameter
*
* \return nothing
*
...
* \brief simple print
*
* do_print just prints the filepath
*
...
/**
*
* \name do_check_parameters
*
* \brief checks if supplied parameters are correct
*
* do_check checks if supplied parameters have their own correct argument
* amounts and if the arguments are valid at all
*
* \param parameters		parameter list
*
* \return SUCCESS if no error occured, otherwise ERROR
...
/**
*
* \brief pre-checks parameters, exits if something is wrong
*        this makes some functions more overseeable
*        and avoids exiting somewhere in the middle of the run
*
...
* \name do_user
*
* \brief checks the file is owned by the user and returns SUCCESS if matching
*
* The function do_user checks if the file is owned by the supplied user
* checks first if the parameter is a username, if that fails it converts it
* to an unsigned long and checks if its a uid. terminates if the user is not
* known to the system
*
* \param buffer		struct stat of the file
* \param parameters	parameter list
* \param offset		value of the offset to the needed parameter
*
* \return SUCCESS if matching, otherwise ERROR
* \retval 0	SUCCESS
* \retval 1	ERROR
*
*/
...
* \name do_name
*
* \brief checks if the last part of the pathname matches the supplied argument
*
* The function do_name checks if the last part of the pathname matches the
* supplied argument and returns true if matching
*
* \param file_name		path to the file
* \param parameters		parameter list
* \param offset			value of the offset to the needed parameter
...
* \name do_no_user
*
* \brief checks if file is unclaimed
*
* do_no_user if file is unclaimed, returns SUCCESS, otherwise ERROR
*
...
/**
*
* \brief checks if file matches specified type
*
* \param file_name full path of the file
* \param type type char (bfcdpls)
*
* \return if type is matching
...
/**
*
* \brief returns type char (bfcdpls) to given mode
*
* \param mode mode of file
*
* \return corresponding type char or default '-'
*
*/
static char get_file_type(const mode_t mode)
{
...
* \name do_path
*
* \brief checks supplied path vs filepath
*
* do_path checks the path pattern against the filepath
*
* \param file_name		path to the file
* \param parameters		parameter list
* \param offset			value of the offset to the needed parameter
